...
class Game:
    ...
    def run(self):
        while True:
            dt = self.clock.tick(S.FPS) / (1000 / S.FPS)
            dt = min(dt, 3.0)
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if self.state == State.MENU:
                    if event.key == pygame.K_UP:
                        self.renderer.menu_prev()
                        self.sound_mgr.play("menu_select")
                    elif event.key == pygame.K_DOWN:
                        self.renderer.menu_next()
                        self.sound_mgr.play("menu_select")
        ...
        if self.state == State.PLAYING:
            ...
            if abs(self.player.anim_tilt) > 2.5 and speed > 4:
                if random.random() < 0.4:
                    ...
                    self.particles.append(Particle(wx, self.player.y + self.player.h - 10, S.C_YELLOW))
                    self.particle_effects.add_spark(wx, self.player.y + self.player.h - 5, S.C_YELLOW, 2)

            # Traînée d'échappement
            if speed > 3:
                self.particle_effects.add_exhaust_trail(
                    self.player.x + self.player.w // 2,
                    self.player.y + self.player.h,
                    speed_ratio
                )
            ...
            for e in self.enemies[:]:
                ...
                if not e.passed and e.y > self.player.y + self.player.h:
                    ...
                    if self.score_mgr.streak >= S.STREAK_MIN:
                        label += f" (COMBO x{self.score_mgr.streak})"
    ...
    def _handle_collision(self, obstacle: bool = False):
        """Gère une collision."""
        ...
        self._shake_timer = 25.0 if obstacle else 20.0
        ...
        for _ in range(12):
            self.particles.append(Particle(
                ...
                self.player.y + self.player.h // 2,
                S.C_MAGENTA))
        self.particle_effects.add_spark(
            self.player.x + self.player.w // 2,
            self.player.y + self.player.h // 2,
            S.C_MAGENTA, 15)
    ...
    def _draw(self):
        if self.state == State.MENU:
            ...
            ox, oy = 0, 0
            if config.get("screen_shake", True) and self._shake_timer > 0:
                mag = min(S.SCREEN_SHAKE_MAX, self._shake_timer * 1.5)
